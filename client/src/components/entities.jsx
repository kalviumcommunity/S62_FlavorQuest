...
import UserFilter from "./UerFilter";
            ...
            {selectedUser===null?(
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                        ...
                        <div
                            ...
                            className="flex flex-col p-6 border border-gray-700 rounded-lg shadow-xl bg-gray-800 hover:shadow-2xl transition-shadow ease-in-out duration-300"
                        >
                            <h3 className="text-2xl font-semibold text-teal-300 mb-3">{selectedUser.name}</h3>
                            <p className="text-gray-300 mb-2">
                                <span className="font-semibold text-teal-400">Age:</span> {selectedUser.age}
                            </p>
                            <p className="text-gray-300">
                                <span className="font-semibold text-teal-400">Description:</span> {selectedUser.description}
                            </p>
                            <p className="text-gray-300">
                                <span className="font-semibold text-teal-400">Created_by:</span> {selectedUser.name}
                            </p>
                            <div className="flex justify-between mt-4">
                                <button
                                    onClick={() => navigate(`/update-entity/${selectedUser._id}`)}
                                    className="bg-yellow-500 text-white py-1 px-4 rounded-lg shadow-md hover:bg-yellow-600 transition-all duration-200"
                                >
                                    Update
                                </button>
